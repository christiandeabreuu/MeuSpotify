    ...
    suspend fun getUserProfileFromApi(accessToken: String): UserProfile? {
        return try {
            Log.d(
                ...
                "Buscando perfil do usuário na API com token: Bearer $accessToken"
            )
            ...
            Log.e(
                "GetUserProfileUseCase",
                "Erro ao buscar dados da API, tentando carregar do banco local: ${e.message}"
            )
